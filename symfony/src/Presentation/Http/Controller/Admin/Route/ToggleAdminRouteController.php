...
final class ToggleAdminRouteController extends AbstractController
{
    #[Route('/api/admin/routes/{id}/toggle', name: 'admin_toggle_route', methods: ['PATCH'])]
    public function __invoke(string $id, EntityManagerInterface $em): JsonResponse
    {
        ...
        $route->adminDisabled = !$route->adminDisabled;
        $em->flush();

        return $this->json(['id' => $route->id, 'adminDisabled' => $route->adminDisabled]);
    }
}
